...
O site é uma **tela azul muito clara** (`--canvas`, `#EDF5FA`) sobre a qual
...
1. **A tela** — azul muito claro `#EDF5FA`, sem fotografias, texturas nem gradiente;
...
A marca tem **duas cores**: **Azul `#2F8EE0`** e **Amarelo `#FFA723`**. Nenhuma
das duas aguenta texto pequeno sobre branco (3,46:1 e 1,94:1), por isso a marca
...
| `--brand-blue` / `--blue-bright` | `#2F8EE0` | **O azul da marca.** 3,46:1 com branco — só superfícies, tintes, gradientes, pins e realces sobre escuro. **Nunca texto pequeno sobre branco.** |
| `--blue-mid` / `--teal` | `#2084D9` | 3,92:1 — ícones, contornos e texto grande sobre a tela. |
| `--blue-text` / `--teal-text` / `--accent-text` | `#1B70B8` | O azul como **texto pequeno** sobre branco (5,18:1). |
| `--accent` / `--primary` / `--ink` / `--blue-deep` | `#1863A3` | **A tinta das ações** — o azul da marca escurecido até dar **6,26:1** com branco. Botões, links, botão principal, texto do item ativo. |
| `--on-accent` | `#ffffff` | Texto branco sobre as superfícies `--ink`. |
| `--accent-hover` / `--ink-hover` / `--blue-deeper` | `#145083` | Hover (tom mais escuro) das superfícies `--ink` (8,38:1). |
| `--blue-darkest` / `--primary-dark` | `#0F3C62` | Blocos escuros e o fim dos gradientes (11,4:1). |
| `--accent-soft` | `rgba(47,142,224,.12)` | Tinte do azul da marca: item de navegação ativo, item de lista ativo, chips. |
| `--gold` / `--amber` | `#FFA723` | **O amarelo da marca** — informação e "ao vivo", nunca ação. **Sempre como fundo**, com `--gold-ink` (`#241500`, 9,14:1) por cima. Nunca com texto branco: daria 1,94:1. |
| `--gold-strong` / `--amber-strong` | `#955900` | A variante para usar como **texto** amarelo sobre branco (5,67:1). |
...
| `--grad-dark` | gradiente `#1863A3 → #0F3C62` | **Blocos de destaque** (banners/CTAs finais) com texto branco. |
| `--grad-sea` / `--grad-sun` | `#2F8EE0 → #1863A3` / `#FFA723 → #E28800` | Gradientes decorativos das duas cores da marca. |
...
| `--blue` / `--sky` / `--focus-ring` | `#1B70B8` | Apenas funcional: links utilitários, "a tua posição" no mapa, o botão de localização e o anel de foco (5,18:1 sobre branco). |
...
**Regra de ouro:** `--ink` (#1863A3) = "a ação desta página" (um botão cheio
por ecrã) e tudo o que é texto pequeno colorido; o **azul da marca** (#2F8EE0)
= seleção e estados ativos, como tinte ou superfície, **nunca com texto branco
nem texto pequeno por cima**; o **amarelo da marca** (#FFA723) = "há vendedores
...
- Foco visível: `outline: 2px solid var(--focus-ring)` (azul de tinta
  `#1B70B8`, 5,18:1 sobre branco, +2px offset) em todos os elementos
  interativos.
...
- Contraste: o site é usado num telemóvel ao sol, onde o mínimo AA não chega.
  ...
  gradiente a variar o fundo. O `--accent` (#1863A3) dá **6,26:1** com
  texto branco, o verde floresta **7,94:1** e o coral **5,58:1**.
  **O azul da marca (#2F8EE0) dá 3,46:1** e **o amarelo da marca (#FFA723)
  dá 1,94:1**: ambos são superfície, nunca texto sobre branco. Como texto
  usam-se `--blue-text` (#1B70B8, 5,18:1) e `--gold-strong` (#955900, 5,67:1);
  para ícones e texto grande, `--blue-mid` (#2084D9, 3,92:1). Não usar
...
A app (`mobile/`) segue **esta direção** — a mesma paleta (azul #2F8EE0 e
amarelo #FFA723), o mesmo vidro e os mesmos neutros frios — mas **mantém a
