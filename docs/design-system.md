...
A marca tem **duas cores**: **Azul `#2AA1B7`** e **Amarelo `#F9B10B`**.
**O azul é um só em todo o site e na app**: botões, links, item ativo, ícones,
contornos e tintes usam `#2AA1B7` (decisão da marca). Só o hover/toque
(`#217E8F`) e os blocos escuros (`#12434D`) usam outro tom. O amarelo não
aguenta texto branco (1,85:1), por isso é sempre fundo com texto escuro.
...
| `--brand-blue` / `--blue-bright` / `--blue-mid` / `--teal` / `--blue-text` / `--teal-text` / `--accent-text` / `--accent` / `--primary` / `--ink` / `--blue-deep` | `#2AA1B7` | **O azul da marca, o único azul.** Botões, links, botão principal, item ativo, ícones, contornos, tintes, gradientes e pins. 3,05:1 com branco: chega para ícones e texto grande/negrito; texto pequeno fica abaixo de AA. |
| `--on-accent` | `#ffffff` | Texto branco sobre as superfícies `--ink`. |
| `--accent-hover` / `--ink-hover` / `--blue-deeper` | `#217E8F` | Hover e toque (tom mais escuro) das superfícies `--ink`. |
...
| `--grad-dark` | gradiente `#2AA1B7 → #12434D` | **Blocos de destaque** (banners/CTAs finais) com texto branco. |
| `--grad-sea` / `--grad-sun` | `#2AA1B7 → #217E8F` / `#F9B10B → #C68B05` | Gradientes decorativos das duas cores da marca. |
...
| `--blue` / `--sky` / `--focus-ring` | `#2AA1B7` | Apenas funcional: links utilitários, "a tua posição" no mapa, o botão de localização e o anel de foco (3,05:1 sobre branco, ≥3:1 exigido para o foco). |
...
**Regra de ouro:** `--ink` (#2AA1B7, o azul da marca) = "a ação desta página"
(um botão cheio por ecrã), a seleção e os estados ativos; o **amarelo da marca** (#F9B10B) = "há vendedores
...
- Foco visível: `outline: 2px solid var(--focus-ring)` (o azul da marca
...
- Contraste: o site é usado num telemóvel ao sol, onde o mínimo AA não chega.
  ...
  gradiente a variar o fundo. O verde floresta dá **7,94:1** com texto
  branco e o coral **5,58:1**. **O azul da marca (#2AA1B7), que é o azul de
  todos os botões, links e ícones, dá 3,05:1**: passa para ícones e texto
  grande/negrito, mas o texto pequeno azul (ou branco sobre ele) fica abaixo
  de AA — é uma escolha da marca. **O amarelo da marca (#F9B10B) dá 1,85:1**:
  é superfície, nunca texto sobre branco; como texto usa-se `--gold-strong`
  (#825C03, 6,03:1). Não usar
...
A app (`mobile/`) segue **esta direção** — a mesma paleta (azul #2AA1B7 e
amarelo #F9B10B), o mesmo vidro e os mesmos neutros frios — mas **mantém a
